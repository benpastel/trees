...
def test_fit_tree():
  ...
  # assert_array_almost_equal(tree.intercepts,    [ 0, 1, 2, 0])
  # assert_array_almost_equal(preds, y)

  # # case: perfect regression
  # # y = 2x + 10
  # X = np.array([9, 2, 7, 0, 2, 100, -3, 100, 100]).reshape((-1, 1))
  # y = np.array([28, 14, 24, 10, 14, 210, 4, 210, 210], dtype=np.double)
  ...
  # assert_array_equal(tree.split_cols,            [0, 0])
  ...
  # assert_array_almost_equal(tree.coefs,          [0, 0, 0, 0])
  # assert_array_almost_equal(tree.intercepts, [0, 1.75, 6.5, 100])
  ...
  #        /       |      \
  #      /         |        \
  #     +1x     +0.5x+5    [20,20,20,20]
  #     |           |
  # [0,1,2,4]     [8,9]
  #
  X = np.array([8, 4, 6, 0, 1, 15, 2, 15, 25, 25]).reshape((-1, 1))
  y = np.array([9, 4, 8, 0, 1, 20, 2, 20, 20, 20], dtype=np.double)
  bins = np.arange(255, dtype=np.float32).reshape((1, -1))
  tree, preds = fit_tree(
    X.T.astype(np.uint8),
    X.T.astype(np.float32),
  ...
  assert_array_equal(tree.left_children,         [1, 4, 5, 0, 0, 0])
  assert_array_equal(tree.mid_children,          [2, 0, 0, 0, 0, 0])
  assert_array_equal(tree.right_children,        [3, 0, 0, 0, 0, 0])
  assert_array_equal(tree.split_cols,            [0, 0, 0, 0, 0, 0])
  assert_array_almost_equal(tree.split_lo_vals,  [4, 0, 0, 0, 0, 0])
  assert_array_almost_equal(tree.split_hi_vals,  [8, 0, 0, 0, 0, 0])
  assert_array_almost_equal(tree.coefs,          [0, 1, 0.5, 0, 0, 0])
  assert_array_almost_equal(tree.intercepts,     [0, 0, 5, 20, 0, 0])
  assert_array_almost_equal(preds, y)
