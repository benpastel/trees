...
static PyObject* build_tree(PyObject *dummy, PyObject *args)
{
    ...
    PyObject *node_mean_obj = PyArray_FROM_OTF(right_children_arg, NPY_DOUBLE, NPY_ARRAY_OUT_ARRAY);
    ...
    double *   restrict node_means     = (double *)   PyArray_DATA((PyArrayObject *) node_mean_arg);

    const npy_intp raw_rows = PyArray_DIM((PyArrayObject *) X_obj, 0);
    const npy_intp raw_cols = PyArray_DIM((PyArrayObject *) X_obj, 1);
    const npy_intp raw_max_nodes = PyArray_DIM((PyArrayObject *) left_children, 0);
    ...
    int node_count = 1;
    const uint16_t memberships [rows]; // the node index each row is assigned to
    const bool should_split   [max_nodes];
    const bool done_splitting [max_nodes];
    ...
    while (node_count < max_node_count && made_a_split) {
        made_a_split = false;
        ...
        for (uint64_t c = 0; c < cols; c++) {
            ...
            for (uint16_t n = 0; n < node_count; n++) {
                ...
                // possibly wasteful - but we needed the totals anyway
                ...
                #pragma omp critical
                {
                    if (col_split_score < node_score[n]) {
                        node_score[n] = col_split_score;
                        node_split_col[n] = c;
                        node_split_val[n] = col_split_val;
                        should_split[n] = true;
                    }
        ...
        for (uint16_t n = 0; n < node_count; n++) {
            if (should_split[n] && new_node_count <= max_node_count - 2) {
                // make the split
        ...
        for (uint64_t r = 0; r < rows; r++) {
            if (should_split[r]) {
                ...
                if (X[r, node_split_col[old_n]] <= node_split_val[old_n]) {
    ...
    // TODO save these
    const int node_counts [node_count];
    const int node_sums   [node_count];
